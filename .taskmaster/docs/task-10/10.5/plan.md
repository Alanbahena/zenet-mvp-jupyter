...
Export `AlignmentAgent` from the public API and create the full test suite so Task 11
can import it cleanly and all Task 10 behavior is verifiable offline.

**Builds on:** 10.4 delivered the full `alineamiento.py` section —
`_load_alignment_context`, `_make_confirm_fn`, and the Gradio UI complete.
**Required by:** 10.6 — documentation and task closure; reads no new code.

---

## Dependencies

- Subtasks 10.1–10.4 done (confirmed in `tasks.json`)
- `ANTHROPIC_API_KEY` in `.env` — required only for 2 live tests; mocked tests run offline
- `pypdf`, `openpyxl` installed (done in 10.4)
...
| Decision | Rationale |
|----------|-----------|
| `AlignmentAgent` added alphabetically to `__all__` | Consistent with ordering in both `__init__.py` files |
| Tests use `unittest.TestCase` + `_MockProvider` | Matches pattern across all existing agent test files |
| `_MockProvider._do_generate` returns fixed JSON string | Avoids real API calls; full abstract signature required |
| `_load_alignment_context` and `_make_confirm_fn` imported directly from `alineamiento.py` | Same pattern as `clasificacion.py` and `configuracion.py` tests |
| SQLite `DataLake` for `recipe_unit_conversion` and confirm tests | `recipe_unit_conversion` is in `_SQLITE_ENTITY_TYPES`; JSON backend doesn't support it |
| Live tests skip without `ANTHROPIC_API_KEY` | `@unittest.skipUnless(os.getenv("ANTHROPIC_API_KEY"), ...)` — consistent with other live tests |
...
| File | Action | Summary |
|------|--------|---------|
| `core/agents/__init__.py` | Modify | Add `AlignmentAgent` import + `__all__` entry |
| `core/__init__.py` | Modify | Add `AlignmentAgent` to agents import block + `__all__` |
...
Add import after `from core.agents.base_agent import BaseAgent`:
```python
from core.agents.alignment_agent import AlignmentAgent
```

Add `"AlignmentAgent"` to `__all__` alphabetically — after `"AgentRegistry"`, before
`"BaseAgent"`.
...
Add `AlignmentAgent` to the agents import block:
```python
...
Add `"AlignmentAgent"` to `__all__` under the `# agents` section after `"AgentRegistry"`.

---

### Step 3 — `tests/unit/test_alignment_agent.py`
...
from core.domain.data_model import (
    CategoryRecipe, FamilyInventory, Ingredient, InventoryItem,
    InventoryUnit, Recipe, RecipeUnit, is_standard_recipe_unit,
)
from core.domain.data_model_utils import ingredients_to_display
from core.domain.serialization import (
    category_recipe_to_dict, family_inventory_to_dict,
    inventory_item_to_dict, inventory_unit_to_dict,
    recipe_unit_conversion_to_dict, recipe_unit_to_dict,
)
from core.operations.normalization import (
    RecipeUnitConversionEntry, RecipeUnitConversionKey,
)
...
class TestAlignmentAgentCore(unittest.TestCase):
    ...
    def test_extracts_recipe_from_plain_text(self):
        ...
        result = agent.run(
            input_data={"user_message": "Mi receta es tacos de pollo", "recipe_source": "conversation", "page_index": 0},
            context={},
        )
    ...
    def test_matches_existing_inventory_item(self):
        ...
        agent, _ = self._make_agent(resp)
        result = agent.run(
            input_data={"user_message": "Sopa de zanahoria", "recipe_source": "conversation", "page_index": 0},
    ...
    def test_proposes_new_inventory_item_with_category_and_family(self):
        ...
        agent, _ = self._make_agent(resp)
        result = agent.run(
            input_data={"user_message": "Pasta con tomate", "recipe_source": "conversation", "page_index": 0},
    ...
    def test_nonstandard_unit_per_ingredient_equivalent(self):
        ...
        agent, _ = self._make_agent(resp)
        result = agent.run(
            input_data={"user_message": "1 taza de harina", "recipe_source": "conversation", "page_index": 0},
            context={},
        )
        ing = result["recipe_draft"]["ingredients"][0]
    ...
    def test_per_ingredient_equivalent_differs_by_ingredient(self):
        ...
        agent, _ = self._make_agent(resp)
        result = agent.run(
            input_data={"user_message": "1 taza harina, 1 taza arroz", "recipe_source": "conversation", "page_index": 0},
            context={},
        )
    ...
    def test_no_hallucination_on_sparse_input(self):
        ...
        agent, _ = self._make_agent(resp)
        result = agent.run(
            input_data={"user_message": "Solo agua", "recipe_source": "conversation", "page_index": 0},
            context={},
        )
        self.assertEqual(len(result["recipe_draft"]["ingredients"]), 1)
    ...
    def test_draft_accumulates_across_turns(self):
        ...
        result1 = agent.run(
            input_data={"user_message": "Se llama Pozole", "recipe_source": "conversation", "page_index": 0},
            context={},
        )
        ...
        result2 = agent2.run(
            input_data={"user_message": "Es una sopa", "recipe_source": "conversation", "page_index": 0},
            context={},
        )
    ...
    def test_create_entity_tool_creates_category(self):
        ...
        # Set up tool prerequisites manually
        agent._data_lake = dl
    ...
    def test_agent_does_not_create_without_confirmation(self):
        dl = _make_data_lake()
        # Mock returns a plain reply with no tool call — agent proposes mapping only
        resp = _alignment_response(reply="¿Quieres usar 'Entradas' o crear una nueva categoría?")
        agent = create_agent(AlignmentAgent, provider=_MockProvider(resp), name="alignment_agent")
        agent.run(
            input_data={"user_message": "Tengo una categoría nueva", "recipe_source": "conversation", "page_index": 0},
    ...
    def test_missing_steps_asks_once_then_saves_none(self):
        # Mock returns recipe without steps
        resp = _alignment_response(
            recipe_name="Guisado",
            recipe_steps=None,
        ...
        agent, _ = self._make_agent(resp)
        result = agent.run(
            input_data={"user_message": "Solo tengo ingredientes", "recipe_source": "conversation", "page_index": 0},
            context={},
        )
...
class TestAlignmentConfirm(unittest.TestCase):
    ...
    def test_confirm_links_ingredient_to_existing_item(self):
        ...
        proposals = [{"name": "Pollo", "category": "Perecedero", "family": None, "status": "matched_existing"}]
        self._run_confirm(dl, draft, proposals)
...
class TestAlignmentPersistence(unittest.TestCase):
    ...
    def test_recipe_unit_conversion_roundtrip(self):
        ...
        entry = RecipeUnitConversionEntry(quantity=120.0, base_unit_id=2, source="agent_confirmed")
        composite_id = f"{key.recipe_unit_id}_{key.family_id}_{key.inventory_item_id}"
        dl.save_entity("recipe_unit_conversion", composite_id, recipe_unit_conversion_to_dict(entry, key))
...
class TestAlignmentUtils(unittest.TestCase):
    ...
    def test_ingredients_to_display_equivalent_and_status_columns(self):
        from core.domain.data_model import Recipe, RecipeUnitRegistry, RecipeUnit
...
class TestAlignmentAgentLive(unittest.TestCase):
    ...
    def test_live_alignment_agent_multi_recipe_pagination(self):
        ...
        self.assertTrue(name0 or name1)  # at least one recipe extracted
...
- `CLAUDE.md` and `tasks.json` status updates — 10.6
- Any changes to `AlignmentAgent` business logic or `alineamiento.py` — prior subtasks closed
- `build_conditional_graph` / `AlignmentGraphState` — 10.3 cancelled

---

## Risks and open questions

### [RISK] — `_make_confirm_fn` and `recipe_unit_conversion` tests require SQLite DataLake
`recipe_unit_conversion` is in `_SQLITE_ENTITY_TYPES`. Tests 10–13 must use
`DataLake(db_path=...)`. Using `DataLake(data_dir=...)` (JSON) will raise or silently
fail. Use `_make_sqlite_data_lake()` helper for those test classes.

### [RISK] — `_create_entity_tool` tests set `_data_lake` and `_context` manually
The tool accesses `getattr(self, "_data_lake", None)` and `getattr(self, "_context", {})`,
which are only set during `_generate_prompt()` inside `agent.run()`. For isolated tool
unit tests (17–19), set `agent._data_lake = dl` and `agent._context = {...}` directly
before calling `agent._create_entity_tool(...)`.

### [RISK] — `tasks.json` says "20 mocked tests" but parent plan test table has 21 rows
Implement 21 per the parent plan test table. The `tasks.json` description is stale.
...
- [ ] `AlignmentAgent` exported from `core/agents/__init__.py`
- [ ] `AlignmentAgent` re-exported from `core/__init__.py`

### `tests/unit/test_alignment_agent.py`
- [ ] 21 mocked tests passing offline
- [ ] 2 live tests skipped without `ANTHROPIC_API_KEY`, passing with key
