...
- **Helpers:**
  - **`make_resolver(get_item)`** — Builds the resolver expected by `normalize_recipe_for_deduction` from a lookup that returns `InventoryItem` per ingredient (e.g. by `Ingredient.inventory_item_id` or by name). Returns a callable that yields (inventory_item_id, family_id, item_unit_id); raises if `get_item(ing)` is None. The resolver’s `get_item` can use **InventoryItemRegistry** (e.g. `registry.get(ingredient.inventory_item_id)` or `registry.get_by_name(ingredient.name)`).
